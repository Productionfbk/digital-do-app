...
with st.form("do_form"):
    ...
    with col2:
        st.session_state.do_date = st.date_input("DO Date", value=st.session_state.do_date)
    ...
    if submitted:
        valid_rows = item_df[(item_df["Item"].str.strip() != "") & (item_df["Quantity"] > 0)]
        ...
        if valid_rows.empty:
            ...
            for _, row in valid_rows.iterrows():
                rows_to_save.append({
                    ...
                    "DO Date": st.session_state.do_date.strftime("%Y-%m-%d"),
            ...
            st.success("✅ DO submitted successfully!")
            st.markdown("### 📄 DO Summary")
            st.write(f"**DO Number:** {st.session_state.do_number}")
            st.write(f"**DO Date:** {st.session_state.do_date}")
...
if os.path.exists(csv_path):
    try:
        ...
        st.error("❌ Ralat membaca fail CSV.")
else:
    st.info("Tiada DO dihantar lagi.")
